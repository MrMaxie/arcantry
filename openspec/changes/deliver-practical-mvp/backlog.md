...
Every original shared queue entry is retained below. Each disposition names the delivered outcome or the part rejected for this MVP. Duplicate ideas share an implementation task.
...
Done: retain standard todo.txt, exact-line capture and existing queue bytes; CLI help and source-based guidance explain the convention boundary. No inferred mandatory taxonomy.
...
Done: concise audience guidance checks source, destination and publication authority without adding another skill.
...
Done: audience guidance uses familiar labels and useful source links; private identifiers remain diagnostic.
...
Done in bounded form: optional focus terms and path exclusions. Rejected for MVP: role questionnaires and an exception-rule language.
...
Done: context, next and explain share core reads with the official Rust MCP SDK; release planning is read-only in MCP.
...
Merged with Q07: configured relevance affects contextual discovery and task priority, never authorization.
...
Done: capture-project-work now names todo capture directly and documents direct-file work.
...
Done: evaluation is an explicit configurator scenario; the homepage leads with the three-command benefit.
...
Done for configured knowledge sources: drift-checked detach retains files, licensing and explicit ownership. Rejected for MVP: arbitrary script reimplementation, package extraction and later-version comparison infrastructure.
...
Done through explain and actual schema templates. Rejected for MVP: an additional overlapping change-authoring skill.
...
Done: --output writes a new plan atomically without overwriting an existing file, with private-content disclosure and platform file permissions.
...
Done for ProjectPlan mutations: root lock, persisted journal, input hashes and explicit recovery review. Existing skill-link rollback remains. Rejected for MVP: a second generalized crash journal for link installation and automatic orphan deletion.
...
Done: Clap owns parsing and help; existing command inventory, JSON plan schema and preview/apply remain the contract. Separate immediate repository init/update/remove commands retain their documented meaning.
...
Done: browser checks of evaluation, existing setup, URL restore, reset, copy feedback, desktop and mobile layout. Rejected for MVP: a new screenshot/performance gating framework.
...
Rejected for MVP: adding a general fuzzing platform duplicates ecosystem parser responsibility. Retained transaction property tests and focused malformed-input, version and privacy cases.
...
Done for context: bounded source reads and active-change count, optional relevance filters. Rejected for MVP: universal time/memory quotas before a measured product bottleneck.
...
Done in bounded form: native synthetic scenarios reproduce discovery, templates, preview/apply, idempotence, drift and privacy on host and Linux. External real-repository benchmark collection is outside MVP.
...
Rejected for MVP: a new catalog taxonomy does not improve the primary workflow enough to justify migration. Keep the catalog and add outcome-led entrypoints.
...
Rejected for MVP: portfolio admission/retirement machinery is separate from shortening the skills needed for daily work.
...
Done: audience-scope-discipline has a concise decision core and retains detailed scenario references.
...
Done: explicit workflow order, dependency blockers, cycle rejection, archive recognition and next --change.
...
Done: all 45 shared entries and 3 private entries have retained provenance and explicit dispositions; queues point to the consolidated work.
...
Done: diagnostics emits allowlisted metadata only, optionally to a new local file, with no upload.
...
Done: text-version@1 and json-version@1 support project identifiers. XML and additional ecosystem adapters are rejected for MVP until needed.
...
Done: the homepage example follows context, next and explain; existing shared/private source visualization remains available. Game mechanics are outside MVP.
...
Done: project/unit SemVer, integer and calendar strategies, compatible sources, exact serialized previews and template rendering.
...
Done: ordinary validation does not require a release; implementation, commit, deployment and publication have separate boundaries.
...
Done: repository guidance compares actual alternatives without option quotas; global third-party skills were not modified.
...
Done: comparisons describe consequences and measured costs instead of invented scores.
...
Done: guidance distinguishes research, recommendation and authorized implementation without a skill-mode questionnaire.
...
Done: Clap, MiniJinja, the official MCP SDK and cargo-llvm-cov replace custom infrastructure; contributor guidance requires ecosystem research and total maintenance-cost comparison.

# Existing OpenSpec changes

The accepted MVP supersedes overlapping scope in these proposals. Their unchecked historical tasks are not evidence of implementation. The dispositions below define which outcomes remain in this MVP; rejected extensions require a new request.

| Existing change | MVP disposition |
| --- | --- |
| add-design-coherent-gui-skill | Existing skill retained; no new design framework. |
| align-host-plugin-identity | Existing portable plugin/package identity retained and locally tested. |
| align-public-trust-surface-with-evidence | Current documentation and native evidence updated; no claim of untested platforms or publication. |
| define-managed-and-detached-adoption | Source ownership transfer implemented; arbitrary asset rewriting and update-comparison machinery rejected for MVP. |
| establish-arcantry-dev-public-domain | Existing local canonical-domain files retained; remote DNS, Pages settings and Cloudflare changes require separate authorization. |
| explain-arcantry-fit-and-adoption-tradeoffs | Outcome-led homepage, source-based start and explicit evaluation scenario implemented. |
| extend-configurator-adoption-scenarios | Evaluation/adoption/existing scenarios and URL restoration implemented. Extra host-specific questionnaire steps removed. |
| harden-native-cli-verification-gates | Native tests retained; CI and coverage floors superseded by local checks and diagnostic coverage. |
| improve-configurator-responsive-reflow | Simpler layout verified at phone and desktop widths. |
| model-audience-based-release-projection | Existing public/internal outcomes retained with configurable templates. New adapter, grouping language and migration subsystem rejected for MVP. |
| preserve-cli-intent-provenance | Unknown conversational authority is explicit; no tool infers approval. New universal intent-policy layer rejected for MVP. |
| preserve-todo-queue-conventions | Exact-line standard todo capture and source guidance retained. Inferred mandatory tags and taxonomy rejected for MVP. |
| recognize-varlock-environment-contracts | Schema discovery and observation-only registration implemented. Environment loading, resolver execution and dotenv fallback rejected: context reads need no secrets. |
