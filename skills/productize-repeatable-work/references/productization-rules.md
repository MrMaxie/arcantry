...
| Mechanical enforcement around tool or lifecycle events | Hook after verifying the supported Codex hook surface |
...
- Private project-only skill: `<repo>/.agents/skills/<name>` with local Git exclusions.
- Global Codex skill: the active Codex home `skills/<name>` entry after validation, installed or linked through the owning catalog workflow.
