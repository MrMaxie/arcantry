...
Recovery runs against the exact project root supplied by `--cwd`. If the interrupted plan included external paths, pass the same exact `--allow-outside` paths to `repo recover`. It reports mixed or edited targets for manual review and only acknowledges a completely original or completely applied result. Staging interruptions before the journal is created can leave unused `.arcantry-*.tmp` files; these never prove ownership and are not automatically removed. The recovery contract covers process interruption, not a guarantee against storage-device or power failure.
