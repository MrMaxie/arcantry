...
<ArcantryMermaid
  label="Arcantry change artifacts from a proposal through specifications, design, tasks, and release metadata"
  diagram={`flowchart LR
    proposal[Proposal] --> specs[Specifications]
    proposal --> design[Design]
    specs --> tasks[Tasks]
    design --> tasks
    proposal --> release[release.md]`}
/>
...
An Arcantry change may be marked `internal`. It remains in the OpenSpec archive, can be assigned when a release is requested, and is omitted only from the public changelog body.

For the general relationship between project authorities and changelog projections, see [Project knowledge stack](/reference/repository-contract/#relationships-and-meaning).
