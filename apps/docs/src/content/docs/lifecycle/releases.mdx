...
Projects using `openspec-release@2` may add `unit_impacts` when the same change has a different SemVer effect in each matching unit. The scalar `impact` remains the fallback. A composed parent change may add `dependency_updates` to acknowledge newer direct dependencies. Impact, visibility, components, unit impacts and dependency updates remain shared by every outcome in the change. Independently acceptable or releasable stories belong in separate OpenSpec changes; one coherent change may still describe several distinct consumer outcomes.
