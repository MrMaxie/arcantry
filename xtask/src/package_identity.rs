...
fn covers_the_declared_release_matrix_and_native_gates() {
  ...
  for expected in [
    "just native-target-check \"${{ matrix.target }}\" \"${{ github.ref_name }}\"",
    "alpine:3.23",
    "installer-smoke:",
    "windows-2025",
    "Smoke installers on their native operating system",
  ] {
    assert!(workflow.contains(expected), "{expected}");
  }
  let compact_workflow = workflow.split_whitespace().collect::<Vec<_>>().join(" ");
  assert!(compact_workflow.contains("publish: needs: - assemble - installer-smoke"));

  let justfile = text("justfile");
  let dockerfile = text("containers/rust-cli-test/Dockerfile");
  let linux_system = text("xtask/src/linux_system.rs");
  assert!(justfile.contains("linux-system-test:"));
  assert!(justfile.contains("cargo run -p xtask -- installer-smoke"));
  assert!(!justfile.contains("tooling/installer-smoke"));
  assert!(
    justfile
      .lines()
      .any(|line| line.starts_with("ci:") && line.contains("linux-system-test"))
  );
  assert!(dockerfile.contains("rust:1.97.1-alpine3.23@sha256:"));
  assert!(linux_system.contains("cargo clippy --workspace --all-targets -- -D warnings"));
  assert!(linux_system.contains("cargo test -p arcantry-cli --test cli_contract"));
}

#[test]
fn documents_the_active_trusted_publishing_workflow() {
  let guide = text("apps/docs/src/content/docs/lifecycle/releases.mdx");
  assert!(guide.contains("workflow `release.yml`"));
  assert!(!guide.contains("workflow `publish-npm.yml`"));
}
